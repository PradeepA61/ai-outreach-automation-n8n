...
## Workflow Nodes
1. Webhook – Receives incoming lead data (username, bio, followers).
2. Edit Fields – Maps the incoming payload into structured fields.
3. If Node – Filters leads based on:
   - Followers greater than 500
   - Bio containing the keyword "AI"
4. AI Agent (LangChain) – Generates a personalized, friendly outreach message using an AI model.
5. Code Node – Parses AI output and formats it into JSON.
6. Edit Fields (Accepted / Rejected) – Adds status, reason, and timestamp for tracking.
7. Google Sheets Append Rows – Stores accepted and rejected leads in separate sheets.
8. Respond to Webhook – Sends back a success or rejection response.
